...
source "$MORGEN_BASH_DEPS/init.bash"
...
declare -rA OPT_LONG=(
    ...
    [DO_NOT_PARSE_BODY]='--no-parse'
)
declare -rA OPT_DESCRIPTION=(
    [INCLUDE_KEYS]='Include the raw keys provided by curl in the output'
    [DO_NOT_PARSE_BODY]='Do not parse the body, even if it would be valid JSON'
)
declare -rA OPT_DEFAULT_VALUE=(
    [INCLUDE_KEYS]=false
    [DO_NOT_PARSE_BODY]=false
)

cli.options.help() {
    >&2 printf 'Usage: %s [<options...>] -- <curl args...>\n\n' "$0"
    >&2 cli.options.help-text
}

cli.options.load "$@"
if ! cli.options.has-positional-args; then
    >&2 printf 'Expected some arguments to pass to curl\n'
    >&2 cli.options.help
    exit 1
fi 
